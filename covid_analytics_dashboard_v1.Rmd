...
summ <- global1 %>%
  group_by(key) %>%
  summarise(freq=sum(val,na.rm=T))%>%
  ungroup()%>%
  mutate(pp=ifelse(key!="Confirmed",freq,NA),
    perc=ifelse(key!="Confirmed",round(pp/sum(pp,na.rm=T)*100,1),NA)) %>% 
  mutate(key=gsub("Confirmed","Total confirmed cases",key),
...
# cc=kenya_plot[kenya_plot$type=="Cumulative cases",]
# 
# plot2=ggplot(cc,aes(x = day_since_first_report, y = cases))+
...
#   scale_x_continuous(breaks  = seq(0,150,10))+scale_y_continuous(breaks  = seq(0,8000,1000))+
#   theme(legend.position="right", legend.title = element_blank(),
#         axis.line.x = element_line(colour="grey"))

#ggsave(file="test.svg", plot=plot2, width=10, height=8)

 
plot1=ggplot(kenya_plot,aes(x = day_since_first_report, y = cases))+
  ...
  geom_vline(aes(xintercept=responses,colour =intervention), 
             linetype="dashed", size=1.5, show.legend = T,na.rm = F)+
  scale_x_continuous(breaks  = seq(0,150,20))+scale_y_continuous(breaks  = seq(0,5000,25000))+
  theme(legend.position="right", legend.title = element_blank())

plot1
ggsave(plot1,file="intervention_chart.png",scale=4)
...
df_africa <- global_data %>%
  # dplyr::filter(date == max(date)) %>%
  dplyr::filter(Country_Region == "Kenya" |Country_Region == "South Sudan" |
    Country_Region == "Tanzania"| Country_Region == "Ethiopia" |Country_Region == "Eritrea"|
    Country_Region == "Uganda" |Country_Region == "Rwanda" | Country_Region == "Djibouti"|
    Country_Region == "Somalia"|Country_Region == "Burundi") %>% 
  select(Country_Region,Confirmed) 
  #spread(Country_Region,cumulative_cases) 
  
  

ggplot(data=df_africa, aes_string(x=paste0("reorder(Country_Region",", Confirmed)"),y="Confirmed")) + coord_flip()+
  geom_bar(stat = "identity",position = position_dodge(),fill = "#CC0000") + 
  my_theme+geom_text(aes(label=paste(round(Confirmed,0),"",sep="")),
   hjust = 0,vjust=-0, size = 4.5,position = position_dodge(1),fontface = "bold")+
    labs(x = "", y = "",title = "Confirmed Covid-19 cases\n(Eastern Africa)")+theme_classic()+my_theme+
  theme(axis.text.x=element_blank(),axis.ticks=element_blank(),
        legend.position ="none")+scale_fill_brewer(palette = "Reds")+ylim(0,50000)
...
ggplot(data=df_africa, aes_string(x=paste0("reorder(Country_Region",", Active)"),y="Active")) + coord_flip()+
  geom_bar(stat = "identity",position = position_dodge(),fill = "#CC0000") + 
  my_theme+geom_text(aes(label=paste(round(Active,0),"",sep="")),
   hjust = 0,vjust=-0, size = 4.5,position = position_dodge(1),fontface = "bold")+
    labs(x = "", y = "",title = "Active Covid-19 cases\n(Eastern Africa)")+theme_classic()+my_theme+
  theme(axis.text.x=element_blank(),axis.ticks=element_blank(),
        legend.position ="none")+scale_fill_brewer(palette = "Reds")+ylim(0,30000)
...
plot1 <- ggplot(data=df_africa, aes(x=day_since_first_report,y=cumulative_cases,
    group=country,color=country))+theme_classic()+geom_line(size=1.)+geom_point(size=.8)+
    labs(x=paste("Days since first case was reported"),y="", title = "Trajectory growth in Cumulative cases\n(Eastern Africa)")+scale_y_continuous(trans='log10')+
     theme(legend.position = "right",
           legend.title = element_blank())+ scale_colour_discrete(na.translate = F)+my_theme+
 scale_x_continuous(breaks =seq(0,150,10))
...
df_africa <- global_data %>%
  # dplyr::filter(date == max(date)) %>%
  dplyr::filter(Country_Region %in% africa) %>% 
  select(Country_Region,Confirmed) %>% 
  arrange(-Confirmed)
  #spread(Country_Region,cumulative_cases) 
  
 df_africa<- df_africa[c(1:10),] 

 top <- as.character(df_africa$Country_Region[1:10]) 

ggplot(data=df_africa, aes_string(x=paste0("reorder(Country_Region",", Confirmed)"),y="Confirmed")) + coord_flip()+
  geom_bar(stat = "identity",position = position_dodge(),fill = "#CC0000") + 
  my_theme+geom_text(aes(label=paste(round(Confirmed,0),"",sep="")),
   hjust = 0,vjust=-0, size = 4,position = position_dodge(1),fontface = "bold")+
    labs(x = "", y = "",title = "Top 10 countries with Confirmed Covid-19 cases\n(Africa)")+theme_classic()+my_theme+
  theme(axis.text.x=element_blank(),axis.ticks=element_blank(),
        legend.position ="none")+scale_fill_brewer(palette = "Reds")+ylim(c(0,700000))
...
ggplot(data=df_africa, aes_string(x=paste0("reorder(Country_Region",", Active)"),y="Active")) + coord_flip()+
  geom_bar(stat = "identity",position = position_dodge(),fill = "#CC0000") + 
  my_theme+geom_text(aes(label=paste(round(Active,0),"",sep="")),
   hjust = 0,vjust=-0, size = 3.5,position = position_dodge(1),fontface = "bold")+
    labs(x = "", y = "",title = "Top 10 countries with Active Covid-19 cases\n(Africa)")+theme_classic()+my_theme+
  theme(axis.text.x=element_blank(),axis.ticks=element_blank(),
        legend.position ="none")+scale_fill_brewer(palette = "Reds")+ylim(c(0,300000))
...
```{r,fig.width=11,rows.print=5,results='asis'}
df_africa <- complete_data %>%
 dplyr::filter(country %in% top|country == "Kenya" ) %>% 
  select(country,cumulative_cases,day_since_first_report,firstcase) %>% 
  mutate(today_kenya=Sys.Date()-date("2020-03-12")-1) %>% 
  filter(day_since_first_report<=today_kenya) %>% 
    filter(day_since_first_report>0) 
...
plot1<- ggplot(data=df_africa, aes(x=day_since_first_report,y=cumulative_cases,
    group=country,color=country))+theme_classic()+geom_line(size=1.)+geom_point(size=.8)+
    labs(x=paste("Days since first case was reported"),y="", title = "Kenya and Top 10 Countries with Confirmed COVID-19 in Africa")+
     theme(legend.position = "right",
           legend.title = element_blank())+ scale_colour_discrete(na.translate = F)+my_theme+
 scale_x_continuous(breaks =seq(0,150,10))
...
plot1<- ggplot(data=df_africa, aes(x=day_since_first_report,y=cumulative_cases,
    group=country,color=country))+theme_classic()+geom_line(size=1.)+geom_point(size=.8)+
    labs(x=paste("Days since first case was reported"),y="", title = "Trajectory growth in cumulative cases in Kenya \nwith comparison to top 10 countries in Africa")+ scale_y_continuous(trans='log10')+
     theme(legend.position = "right",
           legend.title = element_blank())+ scale_colour_discrete(na.translate = F)+my_theme+
 scale_x_continuous(breaks =seq(0,150,10))
...
# #### interactive map
# 
# bin <- c(1,5,10,20,50,100,200,500,800,1000,2000)
# 
...
knitr::include_graphics("C:/Users/Admin/Documents/naominganga.github.io/map/infographics/covid19 cases as of 23rd August 2020.jpeg")
```

### **Nairobi county cases** 

```{r, echo=FALSE, fig.cap="A caption", out.width = '100%'}
knitr::include_graphics("C:/Users/Admin/Documents/naominganga.github.io/map/infographics/Spread_of_covid_19Nairobi_11th_August_2020.jpg")

```


### **Mombasa county cases** 

```{r, echo=FALSE, fig.cap="A caption", out.width = '100%'}
knitr::include_graphics("C:/Users/Admin/Documents/naominganga.github.io/map/infographics/Spread_of_covid_19_Mombasa_11th_August_2020.jpg")
