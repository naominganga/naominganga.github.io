...
base.dir= "C:/Users/Admin/Documents/naominganga.github.io/covid19"
...
# 
full_data <- read.csv("C:/Users/Admin/Documents/naominganga.github.io/covid19/covid19countryinfo.csv",stringsAsFactors = F)
...
intervention_data<-full_data %>% 
  dplyr::select(country,quarantine,schools,publicplace,gatheringlimit,gathering,nonessential,curfew,cessation,golf_clubs,face_masks,hospibed,smokers,sexratio,density,firstcase,totalcases,deaths) %>% 
...
dates_1= c("quarantine","schools","publicplace","gathering","nonessential","firstcase","curfew","cessation","golf_clubs","face_masks")
...
to_plot=complete_data %>% 
  dplyr::select(country,day_since_first_report,cumulative_cases,deaths,recovered,
                quarantine_interv,schools_interv,publicplace_interv,gathering_interv,nonessential_interv,curfew_interv,cessation_intv,golf_clubs_interv,face_masks_interv)
...
kenya_plot<- to_plot %>% 
  filter(country=="Kenya") %>% 
mutate(intervention=plyr::mapvalues(intervention,c("School Closure","Public place restriction","Gathering restriction","Quarantine","Curfew","Movement cessation in affected counties","Closure of all golf clubs","Mandatory wearing of masks in public places"),c(1,2,3,4,5,6,7,8))) %>% 
 mutate(intervention=factor(intervention,levels = c(1,2,3,4,5,6,7,8),labels = c("School Closure","Public place restriction","Gathering restriction","Quarantine","Curfew","Movement cessation in\naffected counties","Closure of all golf clubs","Mandatory wearing of masks in public places")))
...
ggplot(kenya_plot,aes(x = day_since_first_report, y = cases))+
  ...
  scale_x_continuous(breaks  = seq(0,34,2))+scale_y_continuous(breaks  = seq(0,max(kenya_plot$cases)+5,30))+
...
### **Deaths**(Proportion of people who have died)
...
### **Recovered**(Proportion of people who have recovered)
...
### **World map of cases (*use + and - icons to zoom in/out*) and click the finger pointer to get the country's the statistics**


```{r,comment=NA,warning=F,error=T,message=F}
# world.shp<- rgdal::readOGR(dsn="C:/Users/Admin/Documents/Icha/Covid19/raw_data/world_map",layer = "Countries0302")
# 
# tomap<- global_data %>% 
#   select(Country_Region,Long_,Lat,Deaths,Recovered,Confirmed) %>% 
#   rename(CNTRY_NAME=Country_Region) %>% 
#   mutate(region=ifelse(CNTRY_NAME %in% africa,"Africa",NA))
# 
# world.shp=sp::merge(world.shp,tomap,by="CNTRY_NAME",duplicateGeoms = T)
...
```{r}
# 
# africa_data <-global_data%>% 
#   mutate(log_cases=log(Confirmed))%>% 
#   mutate(region=ifelse(Country_Region %in% africa,"Africa",NA)) %>% 
#   filter(region=="Africa")
# 
# pal <- colorFactor(c("grey", "red"),
#                    domain = unique(africa_data$Confirmed))
# 
# 
# (map <- leaflet() %>%
#    #setView(1, -1, zoom =9) %>% 
